		...
		<div class="col-md-12">
			...
			<table class="table table-bordered table-hover"
				...
				<thead>
					<tr>
						<th
							style="background-color: #fafafa; color: #000000; width: auto;"><h5>투표
								종류</h5></th>
						<th
				...
				<tbody>
					<c:choose>
						<c:when test="${empty voteApplyList}">
							<tr>
								<td colspan="5" style="vertical: middle;">게시된 글이 없습니다.</td>
							</tr>
						</c:when>
						<c:otherwise>
								...
								<TR>
									...
									<TD style="width: 50px; vertical-align: middle;">
										<button type="button" class="btn btn-primary"
											onclick="window.location.href='./applyVote?voteKind=${voteApplyL.voteKind}'">시작</button>
									</TD>
									<td style="width: 50px; vertical: middle;"><button type="button"
											class="btn btn-warning"
											onclick="window.location.href='./deleteVoteApply?voteKind=${voteApplyL.voteKind}'">삭제</button></td>
								</TR>
							</c:forEach>
						</c:otherwise>
					</c:choose>
			...
			<button type="button" class="btn btn-primary pull-right"
				onclick="window.location.href='./voteKindApply.jsp'">신청하기</button>
			<div class="col-md-12">
				<div class="text-center">
					<ul class="pagination">
						<c:if test="${pageNumber ne '1'}">
							<li><a href="./voteApplyView?pageNumber=${pageNumber-1}">이전</a></li>
						</c:if>
						<c:if test="${nextPage}">
							<li><a href="./voteApplyView?pageNumber=${pageNumber+1}">다음</a></li>
						</c:if>
					</ul>
				</div>
			</div>
