					...
					<tr>
						...
						<th
							style="background-color: #fafafa; color: #000000; width: 150px;"><h5>종료
								일</h5></th>
						<th
							style="background-color: #fafafa; color: #000000; width: 100px;"><h5>투표
								수</h5></th>
						<th
							style="background-color: #fafafa; color: #000000; width: 100px;"><h5>결과
								확인</h5></th>
						<th
							style="background-color: #fafafa; color: #000000; width: 50px;"><h5>삭제</h5></th>
					</tr>
				</thead>
				<tbody>
					<c:choose>
						<c:when test="${empty endVoteList}">
							<tr>
								<td colspan="4">게시된 글이 없습니다.</td>
							</tr>
						</c:when>
						<c:otherwise>
							<c:forEach var="list" items="${endVoteList}">
								<TR>
									<TD>${list.voteKind}</TD>
									<TD>${list.endDate}&nbsp;${list.endTime}</TD>
									<td>${list.getVote}</td>
									<td><a href="./endVoteResult?voteKind=${list.voteKind}">결과
											확인</a></td>
									<td><button type="button" class="btn btn-warning"
											onclick="window.location.href='./deleteVoted?voteKind=${list.voteKind}'">삭제</button></td>
								</TR>
							</c:forEach>
						</c:otherwise>
					</c:choose>
				...
				<div class="text-center">
					<ul class="pagination">
						<c:if test="${pageNumber ne '1'}">
							<li><a href="./endVoteView?pageNumber=${pageNumber-1}">이전</a></li>
						</c:if>
						<c:if test="${nextPage}">
							<li><a href="./endVoteView?pageNumber=${pageNumber+1}">다음</a></li>
						</c:if>
					</ul>
				</div>
			</div>
