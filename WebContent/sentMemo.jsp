		...
		<div class="container content">
			...
			<div class="col-md-12">
				<div class="table-responsive">
					<form method="post" action="./deleteSentMemoSelected">
						<table class="table table-bordered table-striped"
							style="width: 100%;">
							<thead>
								<tr>
									<th colspan="4">보낸 쪽지함</th>
								</tr>
								<tr>
									<th style="width: 45%;">제목</th>
									<th style="width: 12%;">받는이</th>
									<th style="width: 13%;">작성일</th>
									<th style="width: 7%;">시간</th>
								</tr>
							</thead>
							<tbody>
								<c:forEach var="item" items="${sentMemoList}">
									<tr>
										<td><input type="checkbox" name="selected"
											value="${item.seqno}"><a
											href="./sentMemoView?realseqno=${item.realseqno}">${item.title}</a></td>
										<td>${item.recv_id}</td>
								...
								<tr>
									<td colspan="4"><input class="btn btn-success pull-right"
				...
				<div class="text-center">
					<ul class="pagination">
						<c:if test="${pageNumber ne '1'}">
							<li><a href="./sentMemo?pageNumber=${pageNumber-1}">이전</a></li>
						</c:if>
						<c:if test="${nextPage}">
							<li><a href="./sentMemo?pageNumber=${pageNumber+1}">다음</a></li>
						</c:if>
					</ul>
				</div>
			</div>
