			...
			<div class="col-md-12">
				<div class="table-responsive">
					<table class="table table-bordered table-striped"
						...
						<thead>
							<tr>
								<th style="font-weight: bold; width: 70px;">번호</th>
								<th style="font-weight: bold; width: 1300px;">제목</th>
								<th style="font-weight: bold; width: 150px;">작성자</th>
								<th style="font-weight: bold; width: 150px;">작성 날짜</th>
								<th style="font-weight: bold; width: 150px;">작성 시간</th>
								<th style="font-weight: bold; width: 150px;">조회수</th>
							</tr>
						</thead>
						<tbody>
							<c:choose>
								<c:when test="${bbsCnt eq 0 }">
									<tr>
								...
								<c:otherwise>
									<c:forEach var="cnt" begin="0" end="${BBS_LIST.listSize-1}">
										<TR>
											...
											<TD><a href="bbs?boardNo=${BBS_LIST.seqNo[cnt]}">${BBS_LIST.title[cnt]}</a>
												<c:if test="${loginedID eq BBS_LIST.writer[cnt] }">
													<a href="bbsDelete?boardNo=${BBS_LIST.seqNo[cnt]}"
														style="color: red;" class="pull-right" type="submit"
														onclick="return confirm('정말로 삭제하시겠습니까?')"><i
														class="glyphicon glyphicon-trash"></i>삭제</a>
													<a href="bbsUpdate?boardNo=${BBS_LIST.seqNo[cnt]}"
														style="color: orange; padding-right: 25px;"
														class="pull-right" type="submit"><i
														class="glyphicon glyphicon-erase"></i>수정</a>
												</c:if></TD>
											<TD>${BBS_LIST.writer[cnt]}</TD>
											<TD>${BBS_LIST.date[cnt]}</TD>
											<TD>${BBS_LIST.time[cnt]}</TD>
											<TD>${BBS_LIST.step[cnt]}</TD>
										</TR>
									</c:forEach>
								</c:otherwise>
							</c:choose>
							<tr>
								<td colspan="6"><c:if
										test="${!firstPage and BBS_LIST.seqNo[BBS_LIST.listSize-1]+bbsCnt <= lastSeqNo}">
										<a
											href="bbslist?LAST_SEQ_NO=${BBS_LIST.seqNo[BBS_LIST.listSize-1]+bbsCnt+15}"
											style="background-color: #fafafa; color: black; border: black;"
											class="btn pull-center" type="submit"><i
											class="glyphicon glyphicon-menu-left"></i>이전 페이지</a>
									</c:if> <c:if test="${!BBS_LIST.lastPage}">
										<a
											href="bbslist?LAST_SEQ_NO=${BBS_LIST.seqNo[BBS_LIST.listSize-1]}"
											style="background-color: #fafafa; color: black; border: black;"
											class="btn btn-custom pull-center" type="submit">다음 페이지<i
											class="glyphicon glyphicon-menu-right"></i></a>
									</c:if> <a href="./CanBBSWriteServlet"
									style="background-color: white;" class="btn pull-right"
									type="submit"><i class="glyphicon glyphicon-pencil"></i>글쓰기</a>
								</td>
							</tr>
						</tbody>
					</table>
